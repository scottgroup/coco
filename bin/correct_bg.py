...
def correct_bed12(df, nb_threads):
    ...
    if pd.__version__ >= '0.23.0':
        df = pd.concat(list(results), sort=False)
    else:
    ...
    if pd.__version__ >= '0.23.0':
        df_bed12 = pd.concat([df_wo_overlap, df_identical, df_same_start, df], sort=False)
    else:
...
def genome_cov(output_dir, temp_dir, output, genomepath, ucsc):
    ...
    if ucsc is True:
        with open(genomepath) as f:
            ...
            line = f.readline()
        if line[0:3] != 'chr':
            v = subprocess.check_output(['awk', '-W', 'version']).decode('utf-8').strip().split()
            if v[0] == "GNU":
                v = v[2].strip(',')
            elif v[0] == 'mawk':
                v = v[1]
